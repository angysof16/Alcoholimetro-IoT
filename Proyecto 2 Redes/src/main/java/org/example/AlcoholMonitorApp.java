...
public class AlcoholMonitorApp {
    ...
    private static final String TOPIC_RECEIVE = "***";
    private static final String TOPIC_BUZZER = "****";
    private static final String CLIENT_ID = "****_" + System.currentTimeMillis();
    ...
    public static void main(String[] args) {
            ...
            try {
                ...
                client.subscribe(TOPIC_RECEIVE, (topic, msg) -> {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        ...
                        if (alcoholLevel > 500 && !buzzerActive && !buzzerOverride) {
                            activateBuzzer(messageArea);
                        }

                        if (alcoholLevel <= 500 && buzzerActive) {
        ...
        sendTelegramButton.addActionListener(e -> {
            try {
                String manualMessage = "Mensaje manual desde el sistema de monitoreo.";
                telegramNotifier.sendMessage(manualMessage);
                messageArea.append("Mensaje enviado a Telegram: " + manualMessage + "\n");
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(frame, "Error al enviar mensaje a Telegram: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            }
        });
